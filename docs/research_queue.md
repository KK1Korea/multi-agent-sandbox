...
* 측정: (1) 다른 LLM(Opus vs Sonnet vs 외부 모델) 간 토의 결과 비교, (2) 동일 LLM 내 편향 상관도 측정
* 성공 기준: 이종 모델 토의가 동종 대비 편향 감소 → 아키텍처 변경 필요. 차이 미미 → 현행 유지.
* 의존: RQ-4(하위 모델 품질), 이종 모델 접근 가능성
* 관련: MasterLog [37], RQ-4, "Bias in the Mirror" ACL 2025
