...
# Sandbox Debate Orchestrator — Cowork_CPAS v0.9.16
...
Architecture: Main (Orchestrator) → {Advocate, Skeptic, MasterLog-Filter, TrueLog-Filter, FailLog-Filter, Research-Queue-Filter} — 2-level only.
...
Spawn 4 filter agents in parallel:
...
Agent tool call:
  ...
  prompt: |
    Debate topic: {TOPIC}
    Read .context/research_queue.md and extract ONLY research questions relevant to the debate topic.
    Preserve cumulative findings (현황) in full — do NOT truncate.
    Identify open questions this debate could help answer.
    Deliver filtered results in your structured format.
```
...
Agent tool calls (ALL in a SINGLE message):
  ...
  - research-queue-filter → .context/research_queue.md (always single file)
```
...
Capture all outputs and merge into TWO separate data blocks:

**`{FILTERED_DATA}` — Skeptic-only internal evidence:**
```
...
★ `{RESEARCH_CONTEXT}`는 `{FILTERED_DATA}`와 별도로 관리한다.
- `{FILTERED_DATA}` → Skeptic에만 전달 (과거 증거, 한쪽 무기)
- `{RESEARCH_CONTEXT}` → Advocate와 Skeptic 양쪽에 전달 (공유 연구 컨텍스트)
- 이유: research_queue는 무기가 아니라 "우리가 뭘 알고 뭘 모르는지"의 공유 기반

### Step 3 — Assess Internal Data Sufficiency

From merged filter outputs (count across log filters — masterlog, truelog, faillog):
- **2+ relevant entries total** → sufficient
- **0-1 relevant entries total** → thin
- **All three report "No relevant data"** → search-only

Research context assessment (separate from above):
- **1+ relevant RQ** → research context available
- **0 relevant RQ** → no research context
- research_queue 유무는 sufficiency 판정에 영향 없음 (독립 평가)
...
1. **Spawn/Resume Advocate:**
   ...
   Agent tool call:
     ...
     prompt: |
       ...
       === RESEARCH CONTEXT (shared — what we know and don't know) ===
       {RESEARCH_CONTEXT}
       ...
       The research context shows what has been studied and what remains open — use it to ground your arguments.
...
5. **Spawn/Resume Skeptic:**
   ...
   Agent tool call:
     ...
     prompt: |
       ...
       The research context shows what has been studied and what remains open — use it to ground your counterarguments.
...
1. **Intermediate Analysis**: Structure issues from Session 1's 8 turns (see Phase 4 format).
...
## Phase 3 — Sandbox_Log 기록 (Observer 역할) ★ v0.9.16 신설

**이 Phase에서 오케스트레이터는 Observer로 전환한다.**
판단/평가/추천 일절 없이, 토의에서 발생한 사실만 기록한다.
토의 완료 즉시, 사용자 동의 없이 자동 실행한다.

### Step 3.1 — Sandbox_Log 자동 저장
...
────────────────────────────────────────
  ...
  ■ 토의 주제: {topic}
  ■ 쟁점: {core issue — 양측이 충돌한 중심 논점, 사실만}
  ■ 평가자(Advocate) 근거: {Advocate가 제시한 주장과 근거 요약, 판단 없이 사실 나열}
  ■ 비평자(Skeptic) 근거: {Skeptic이 제시한 반론과 근거 요약, 판단 없이 사실 나열}
  ■ 핵심 차이: {양측을 가르는 핵심 변수, 1줄}
  ■ 미검증: {양측 모두 검증하지 못한 항목}
  ■ 분리 쟁점: {토의 중 파생된 별도 쟁점}
  ■ RQ 기록: {이 토의에서 신규/갱신된 RQ 목록 — RQ-N 형태}

```

**Observer 기록 원칙:**
- **무판단**: "어느 쪽이 강하다", "이 방향을 추천한다" 등 오케스트레이터 사견 일절 금지
- **사실만**: 양측이 실제로 말한 내용의 요약만 기록. 해석/평가/품질 판정 없음
- **RQ 기록 즉시 반영**: Step 3.2에서 research_queue 업데이트 완료 후 Phase 4 진행
...
### Step 3.2 — Research Queue 즉시 업데이트

Sandbox_Log 기록 직후, `.context/research_queue.md`를 즉시 업데이트한다.
**Phase 4 진행 전에 완료해야 한다.**

1. Extract from Sandbox_Log의 이번 엔트리:
   - ■ 미검증 항목 → each becomes a research queue entry
   - ■ 분리 쟁점 → each becomes a research queue entry
   - 토의 중 제안되었으나 검증되지 않은 반증 조건
...
**Step 3.2 완료 후 Phase 4로 진행한다.**

## Phase 4 — 오케스트레이터 최종 보고서 (기존 Phase 3에서 이동)

여기서부터 오케스트레이터 고유 역할로 복귀한다.
시계열 분석, 품질 판정, 오케스트레이터 평가 등 판단을 포함한 보고서를 사용자에게 출력한다.
