...
2. [RQ-2] D-5(conflicted), D-2(mixed/Advocate 우위) 완료. 12건 중 non-clear 6건(50%) — 성공 기준 ≥25% 충족. 단, D-5/D-2 모두 외부 데이터 오케스트레이터 직접 주입 경로 → 정상 필터 경로와 신뢰도 차이 있음. 추가 분류 정밀화는 스킬 정상 운영 시 자연 축적 데이터로 수행 (HIGH)
