...
────────────────────────────────────────
  ...
  ■ 교훈:
    - cpu-anchor 프로젝트에서 3일간 41커밋 삽질한 경험 → CPAS 개발 동기
    - 실험 설계 단계에서 batch size confound를 잡을 수 있었으면 3일치 삽질 회피 가능
    - CPAS의 가치는 "분석 품질"보다 "삽질 방지" — 잘못된 방향에 시간을 쏟기 전에 구조화된 반론으로 검증
    - Opus 단독으로 충분한 경우: 맥락 탐색, 아이디어 확장, 비가역적이지 않은 결정
    - CPAS가 필요한 경우: 되돌리기 어려운 결정, 기존 반론이 부족한 미지 영역, 자기확신 방지

  태그: [구상]
  관련: [33], [28], RQ-1, True_Log [31][32], cpu-anchor 프로젝트
